...
var $return_data = "";
  ...
  function Gravatar()
  {
    ...
    $rating = $TMPL->fetch_param('rating');
    $size = $TMPL->fetch_param('size');
    $default = $TMPL->fetch_param('default');
    $email = $TMPL->fetch_param('email');
    $name = $TMPL->fetch_param('name');
    if ($email == "" && $name != "")
    {
	  $sql = "SELECT email FROM exp_members WHERE screen_name = '".$DB->escape_str($name)."'";
	  $query = $DB->query($sql);
	 @$results = $query->row['email'];
